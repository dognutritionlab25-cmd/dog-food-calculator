...
with col1:
    ...
    activity = st.selectbox("활동량", [1.0, 1.2, 1.4, 1.6, 2.0, 3.0], index=3)
    rer = 70 * (weight ** 0.75)
    der = rer * activity
    ...
    with c2:
        ...
        if total_kcal > 0:
            ...
            for nutri, std in aafco_standards.items():
                ...
                status = "✅ 적합"
                if val_1000 < min_v: status = f"❌ 부족 (최소 {min_v})"
                elif max_v and val_1000 > max_v: status = f"⚠️ 과잉 (최대 {max_v})"
